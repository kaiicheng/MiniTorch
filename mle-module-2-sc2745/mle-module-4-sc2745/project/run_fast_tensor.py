...
class Network(minitorch.Module):
    ...
    def forward(self, x):
        ...
        # same as run_tensor.py task 2.5
        out = self.layer1.forward(x).relu()
        end = self.layer2.forward(out).relu()
...
class Linear(minitorch.Module):
    ...
    def forward(self, x):
        # TODO: Implement for Task 3.5.
        matrix_mul = x @ self.weights.value
        bias = self.bias.value.view(1, self.bias.value.shape[0])
        return matrix_mul + bias
        # raise NotImplementedError("Need to implement for Task 3.5")
