...
def map(fn: Callable[[float], float]) -> Callable[[Iterable[float]], Iterable[float]]:
    ...
    # return a function, so create another function
...
def reduce(
    ...
    def fn_new(ls: Iterable[float]) -> float:
        ...
        for x in ls:
            result = fn(result, x)
        # for x in ls:
        #     result = fn(x, result)  # x1 + x0 -> res / x2 + res -> res / x3 + res -> res / res
        return result
...
def sum(ls: Iterable[float]) -> float:
    ...
    # TODO: Implement for Task 0.3.
    # use built reduce()
    # start to loop over from the first element, so starts from 0
    result = reduce(add, 0)
    # result = reduce(add, 0)(ls)
    return result(ls)
...
def prod(ls: Iterable[float]) -> float:
    ...
    # TODO: Implement for Task 0.3.
    # use built reduce()
    # start to loop over from the first element, so starts by multiplying 0
