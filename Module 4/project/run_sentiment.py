...
class Conv1d(minitorch.Module):
    ...
    def forward(self, input):
        ...
        result = minitorch.conv1d(input, self.weights.value) + self.bias.value
        return result
...
class CNNSentimentKim(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        out_1 = self.conv1.forward(embeddings).relu()
        out_2 = self.conv2.forward(embeddings).relu()
        out_3 = self.conv3.forward(embeddings).relu()
        result = minitorch.nn.max(out_1, 2) + minitorch.nn.max(out_2, 2) + minitorch.nn.max(out_3, 2)

        # ValueError: too many values to unpack (expected 2)
        result = result.view(result.shape[0], result.shape[1])

        out = self.linear.forward(result)
