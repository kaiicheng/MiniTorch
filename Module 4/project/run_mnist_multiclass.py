...
class Conv2d(minitorch.Module):
    ...
    def forward(self, input):
        ...
        result = minitorch.conv2d(input, self.weights.value)
...
class Network(minitorch.Module):
    ...
    def __init__(self):
        ...
        # TODO: Implement for Task 4.5.
        # 4 output channels and a 3x3 kernel
        self.Layer1 = Conv2d(1, 4, 3, 3)
        # 8 output channels and a 3x3 kernel
        self.Layer2 = Conv2d(4, 8, 3, 3)
        # Should be size BATCHx392
    ...
    def forward(self, x):
        ...
        # 3. (16, 8, 7, 7)
        temp = minitorch.nn.avgpool2d(self.out, (4, 4))

        # 4. (16, 392)
        temp = temp.view(BATCH, temp._tensor.shape[1] * temp._tensor.shape[2] * temp._tensor.shape[3])

        # 5. (16, 4)
        temp = self.Layer3.forward(temp).relu()
        temp = minitorch.nn.dropout(temp, 0.25)

        # 6. (16, 10)
        temp = self.Layer4.forward(temp)

        # 7. (16, 10)
        result = minitorch.nn.logsoftmax(temp, 1)
