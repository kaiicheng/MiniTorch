    ...
    def _map(
        ...
        if (len(out_strides) != len(in_strides)
            ...
            for i in prange(len(out)):
                ...
                # broadcast to the index in the input
                broadcast_index(out_index, out_shape, in_shape, in_index)

                # apply the function to the input and store in the output
    ...
    def _zip(
        ...
        if (len(out_strides) != len(a_strides)
            ...
            for i in prange(len(out)):
                ...
                # broadcast to the index in each input
                out_ordinal = index_to_position(out_index, out_strides)
    ...
    * Inner-loop should not call any functions or write non-local variables
    ...
    def _reduce(
        ...
        for i in prange(len(out)) :
            ...
            # calculate the index of an output
            out_index = np.empty(MAX_DIMS, np.int32)
            reduce_size = a_shape[reduce_dim]
            to_index(i, out_shape, out_index)

            # calculate out_ordinal with index_to_position
            out_ordinal = index_to_position(out_index, out_strides)
            sum = out[out_ordinal]

            # calculate a_ordinal with index_to_position
            a_ordinal = index_to_position(out_index, a_strides)
            steps = a_strides[reduce_dim]

            # loop over and apply function to the input
            for j in range(reduce_size):
                sum = fn(sum, a_storage[a_ordinal])
                a_ordinal += steps
    ...
    for i in prange(out_shape[0]):
        for j in prange(out_shape[1]):
            for k in prange(out_shape[2]):
                ...
                # calculate a and b matrix multiplication
                ...
                # position after calculating
                result = i * out_strides[0] + j * out_strides[1]\
                    + k * out_strides[2]
